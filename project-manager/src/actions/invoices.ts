...
// ─── Add Invoice Items (Tansiq) after invoice creation ───
export async function addInvoiceItems(invoiceId: string, items: Array<{
    name: string;
    itemNumber?: string;
    description?: string;
    quantity: number;
    ...
    try {
        ...
        if (invoice.creatorId !== session.id && session.role !== "ADMIN") {
            return { error: "غير مصرح لك بتعديل هذه الفاتورة" };
        }
        if (invoice.status !== "PENDING") {
            return { error: "لا يمكن تعديل فاتورة تمت معالجتها" };
        }
        ...
        return { error: "حدث خطأ أثناء إضافة البنود" };
    }
}
...
export async function deleteInvoice(id: string) {
    try {
        ...
        if (!session || session.role !== "ADMIN") {
            return { error: "فقط المدير يمكنه حذف الفواتير" };
        }

        const invoice = await prisma.invoice.findUnique({
            where: { id },
            include: { project: true, outOfPocketDebt: true }
        });

        if (!invoice) return { error: "الفاتورة غير موجودة" };

        if (invoice.projectId && invoice.project?.managerId !== session.id) {
            return { error: "لا يمكنك حذف فواتير مشروع لا تديره" };
        }

        // I7: If the invoice was APPROVED, reverse all balance effects before soft-deleting
        if (invoice.status === "APPROVED") {
            const isEmployeeCustody = !!invoice.custodyId && (invoice.paymentSource === "CUSTODY" || invoice.paymentSource === "SPLIT");
            const isManagerImplicit = !invoice.custodyId && invoice.projectId &&
                (invoice.paymentSource === "CUSTODY" || invoice.paymentSource === "SPLIT");

            const deductAmount = invoice.paymentSource === "SPLIT"
                ? (invoice.custodyAmount ?? invoice.amount)
                : invoice.amount;

            if (isEmployeeCustody) {
                // Restore employee custody balance
                await prisma.employeeCustody.update({
                ...
                if (invoice.projectId) {
                    await prisma.projectMember.updateMany({
                        where: { projectId: invoice.projectId, userId: invoice.creatorId },
                        data: { custodyBalance: { increment: deductAmount } }
                    });
                }
            }

            if (isManagerImplicit) {
                // Restore project.managerSpent
                const project = await prisma.project.findUnique({
                    where: { id: invoice.projectId! },
                    select: { managerId: true }
                });
                if (project?.managerId === invoice.creatorId) {
                    await prisma.project.update({
                        where: { id: invoice.projectId! },
                        data: { managerSpent: { decrement: deductAmount } }
                    });
                }
            }

            // Cancel any OutOfPocketDebt (employee no longer owed reimbursement)
            if (invoice.outOfPocketDebt) {
                await prisma.outOfPocketDebt.update({
                    where: { id: invoice.outOfPocketDebt.id },
                    data: { isSettled: true, settledAt: new Date(), settledBy: session.id }
                });
            }
        }
